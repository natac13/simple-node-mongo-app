...
(function () {
  ...
  const apiUrl = 'http://localhost:3300/api/:id';
  ...
  ready(request('get', apiUrl, function(data) {
    ...
    if (profileId) {
      updateHtmlElement(data, profileId, 'profileId');
    }

    if (profileUsername) {
      updateHtmlElement(data, profileUsername, 'profileUsername');
    }

    if ( profileRepos) {
      updateHtmlElement(data, profileRepos, 'profileRepos');
    }
  }))
})();
